...
def create_tray_icon():
    ...
    restart_action.triggered.connect(lambda: new_game_control.start_game_control())
    restart_action.setEnabled(not new_game_control.is_game_control_running)  # 초기 상태 설정

    # 메뉴를 트레이 아이콘에 설정
    tray_icon.setContextMenu(menu)
    # 10초에 한번씩 update_game_time을(REFRESH_TIME)호출하여 툴팁을 변경한다
    # 게임 제어시작
    new_game_control.start_game_control()
    update_timer = QtCore.QTimer()
    update_timer.timeout.connect(lambda: timer_handler(tray_icon, restart_action))
...
def timer_handler(tray_icon, restart_action):
    new_game_control.check_game_run_time()
    update_game_time(tray_icon, restart_action)
...
def update_game_time(tray_icon, restart_action):
    ...
    restart_action.setEnabled(not new_game_control.is_game_control_running)


def get_game_time():

    hours, minutes, seconds = seconds_to_hours_minutes(new_game_control.game_run_time)
...
def get_remain_game_time():
    ...
    current_game_run_time = new_game_control.game_run_time

    remain_game_run_time = new_game_control.max_game_run_time - current_game_run_time
